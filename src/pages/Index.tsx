...
import { Type, ImageIcon, Calculator, ArrowLeftRight, Binary, Globe, Code, Package, CirclePercent, Percent, Dice5, Banknote, ChartBar, Clock, CalendarDays, Calendar, Utensils } from 'lucide-react';
...
const categories: ToolCategory[] = [{
  ...
  tools: [{
    ...
    description: 'Calculate cost per thousand impressions for advertising',
...
const Index: React.FC = () => {
  return <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {categories.map(category => <div key={category.id} className="col-span-1">
            <div className={`rounded-lg p-0.5 ${category.gradient}`}>
              <div className="bg-card rounded-[calc(0.5rem-1px)] p-6 h-full grid">
                ...
                <ul className="space-y-3">
                  {category.tools.map(tool => <li key={tool.id}>
                      {tool.implemented ? <Link to={tool.path} className="tool-card block p-3 rounded-md border hover:bg-muted/50 transition-all">
                          <div className="font-medium">{tool.title}</div>
                          <div className="text-sm text-muted-foreground">{tool.description}</div>
                        </Link> : <div className="block p-3 rounded-md border bg-muted/30 opacity-60 cursor-not-allowed">
