...
const units: Unit[] = [
  ...
  { 
    id: 'kcal', 
    name: 'Kilocalorie (kcal, food calorie)', 
  ...
  { 
    ...
    fromBase: (value) => value / 3600000 
  },
  { 
    id: 'ev', 
    name: 'Electron-volt (eV)', 
    toBase: (value) => value * 1.602176634e-19, 
    fromBase: (value) => value / 1.602176634e-19 
  },
  { 
    id: 'btu', 
    name: 'British Thermal Unit (BTU)', 
    toBase: (value) => value * 1055.056, 
    fromBase: (value) => value / 1055.056 
  },
  { 
    id: 'ftlb', 
    name: 'Foot-pound (ft·lb)', 
...
const EnergyConverter: React.FC = () => {
  ...
  const [sourceUnit, setSourceUnit] = useState<string>('j');
  const [targetUnit, setTargetUnit] = useState<string>('cal');
  ...
  const convertSourceToTarget = (value: string, from: string, to: string) => {
    ...
    // Convert source value to base unit (Joule), then to target unit
  ...
  const convertTargetToSource = (value: string, from: string, to: string) => {
    ...
    // Convert target value to base unit (Joule), then to source unit
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              <Button 
                ...
                onClick={() => {
                  setSourceUnit('j');
                  setTargetUnit('cal');
                  setSourceValue('4.184');
                  setLastChanged('source');
                }}
              >
                4.184 Joules to Calories
              </Button>
              <Button 
                variant="outline" 
                size="sm"
